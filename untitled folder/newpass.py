...
bg_canvas = tk.Canvas(window, bg=BG, highlightthickness=0, bd=0)
...
def _draw_gradient_background(event=None):
    ...
    # vertical gradient, drawn in thin bands for performance
    step = 4
    for y in range(0, h, step):
        ...
        bg_canvas.create_rectangle(0, y, w, y + step, fill=color, outline="")
    ...
    def glow(cx, cy, radius, color, base_t):
        ...
        for i in range(steps, 0, -1):
            ...
            bg_canvas.create_oval(cx - r, cy - r, cx + r, cy + r,
                                   fill=blend, outline="")

    glow(w * 0.82, h * 0.06, max(w, h) * 0.42, GLOW_BLUE, 0.05)
    glow(w * 0.1, h * 0.95, max(w, h) * 0.46, GLOW_PURPLE, 0.9)
...
bg_canvas.bind("<Configure>", _schedule_bg_redraw)
